	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="评分日期" prop="rating_date">
								<el-date-picker :disabled="disabledObj['rating_date_isDisabled']" v-if="user_group === '管理员' || (form['bonus_items_id'] && $check_field('set','rating_date')) || (!form['bonus_items_id'] && $check_field('add','rating_date'))" id="rating_date"
				...
				<el-form-item label="校区名称" prop="campus_name">
							  ...
							  v-if="user_group === '管理员' || (form['bonus_items_id'] && $check_field('set','campus_name')) || (!form['bonus_items_id'] && $check_field('add','campus_name'))" :disabled="disabledObj['campus_name_isDisabled']"></el-input>
				...
				<el-form-item label="年级名称" prop="grade_name">
							  ...
							  v-if="user_group === '管理员' || (form['bonus_items_id'] && $check_field('set','grade_name')) || (!form['bonus_items_id'] && $check_field('add','grade_name'))" :disabled="disabledObj['grade_name_isDisabled']"></el-input>
				...
				<el-form-item label="班级名称" prop="class_name">
							  ...
							  v-if="user_group === '管理员' || (form['bonus_items_id'] && $check_field('set','class_name')) || (!form['bonus_items_id'] && $check_field('add','class_name'))" :disabled="disabledObj['class_name_isDisabled']"></el-input>
				...
				<el-form-item label="学生" prop="student">
													<el-select v-if="user_group === '管理员' || (form['bonus_items_id'] && $check_field('set','student')) || (!form['bonus_items_id'] && $check_field('add','student'))" id="student" v-model="form['student']" :disabled="disabledObj['student_isDisabled']">
				...
				<el-form-item label="学号" prop="student_id">
							  ...
							  v-if="user_group === '管理员' || (form['bonus_items_id'] && $check_field('set','student_id')) || (!form['bonus_items_id'] && $check_field('add','student_id'))" :disabled="disabledObj['student_id_isDisabled']"></el-input>
				...
				<el-form-item label="评分类型" prop="rating_type">
						...
						v-if="user_group === '管理员' || (form['bonus_items_id'] && $check_field('set','rating_type')) || (!form['bonus_items_id'] && $check_field('add','rating_type'))">
				...
				<el-form-item label="加分分数" prop="bonus_points">
								<el-input-number id="bonus_points" v-model.number="form['bonus_points']"
						v-if="user_group === '管理员' || (form['bonus_items_id'] && $check_field('set','bonus_points')) || (!form['bonus_items_id'] && $check_field('add','bonus_points'))" :disabled="disabledObj['bonus_points_isDisabled']"></el-input-number>
				...
				<el-form-item label="备注信息" prop="remarks">
							  ...
							  v-if="user_group === '管理员' || (form['bonus_items_id'] && $check_field('set','remarks')) || (!form['bonus_items_id'] && $check_field('add','remarks'))" :disabled="disabledObj['remarks_isDisabled']"></el-input>
				...
				<el-form-item label="评分教师" prop="rating_teacher">
							...
							<!--v-if="user_group === '管理员' || (form['bonus_items_id'] && $check_field('set','rating_teacher')) || (!form['bonus_items_id'] && $check_field('add','rating_teacher'))" :disabled="disabledObj['rating_teacher_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','rating_teacher')">{{form['rating_teacher']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['bonus_items_id'] && $check_field('set','rating_teacher')) || (!form['bonus_items_id'] && $check_field('add','rating_teacher'))" id="rating_teacher" v-model="form['rating_teacher']" :disabled="disabledObj['rating_teacher_isDisabled']">
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "bonus_items_id",
				url_add: "~/api/bonus_items/add?",
				url_set: "~/api/bonus_items/set?",
				url_get_obj: "~/api/bonus_items/get_obj?",
				url_upload: "~/api/bonus_items/upload?",
		...
		methods: {
          ...
          this.form["rating_date"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/bonus_items/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/bonus_items/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/bonus_items/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/bonus_items/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/bonus_items/view','get');
