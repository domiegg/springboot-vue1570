				...
				<el-col>
					<el-form :model="form" ref="form" label-width="80px" class="form">
						...
						<el-form-item label="身份" prop="user_group">
							...
							<!-- 教师 -->
						<form_teacher v-if="form.user_group === '教师'" :form="form_sub" ></form_teacher>
								<!-- 学生 -->
						<form_student v-if="form.user_group === '学生'" :form="form_sub" ></form_student>
...
<script>
	import mixin from "@/mixins/page.js";
		/* 教师 */
	import form_teacher from "@/components/diy/form_teacher";
			/* 学生 */
	import form_student from "@/components/diy/form_student";
			...
			async submit_check(param){
				...
				if(!ret && user_group){
						...
						var form_sub = Object.assign({} ,this.form_sub);
											if(!ret && !form_sub["job_no"]){
							ret = "工号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/teacher/count?", {"job_no": form_sub["job_no"]});
							if(res.result){
								ret = "工号已存在!";
							}
						}
							...
							if(user_group === "学生"){
						var form_sub = Object.assign({} ,this.form_sub);
											if(!ret && !form_sub["student_number"]){
							ret = "学号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/student/count?", {"student_number": form_sub["student_number"]});
							if(res.result){
								ret = "学号已存在!";
							}
						}
