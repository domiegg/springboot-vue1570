	...
	<el-main class="bg table_wrap">
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item label="评分日期">
									<el-input v-model="query.rating_date"></el-input>
								</el-form-item>
				</el-col>
																								<el-col :xs="24" :sm="24" :lg="8" class="el_form_search_wrap">
					<el-form-item label="评分类型">
									<el-select v-model="query.rating_type">
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/deduction_items/table','add') || $check_action('/deduction_items/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/deduction_items/table','del') || $check_action('/deduction_items/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
				...
				<el-table-column prop="rating_date" @sort-change="$sortChange" label="评分日期"
				v-if="user_group == '管理员' || $check_field('get','rating_date')" min-width="200">
		                <template slot-scope="scope">
                	{{ $toTime(scope.row["rating_date"],"yyyy-MM-dd") }}
                </template>
					</el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','student_id')" min-width="200">
					...
					<el-table-column prop="rating_type" @sort-change="$sortChange" label="评分类型"
				v-if="user_group == '管理员' || $check_field('get','rating_type')" min-width="200">
					</el-table-column>
					<el-table-column prop="deduction_points" @sort-change="$sortChange" label="扣分分数"
				...
				v-if="user_group == '管理员' || $check_field('get','remarks')" min-width="200">
					...
					<el-table-column prop="rating_teacher" @sort-change="$sortChange" label="评分教师"
				v-if="user_group == '管理员' || $check_field('get','rating_teacher')" min-width="200">
						<template slot-scope="scope">
					{{ get_user_rating_teacher(scope.row['rating_teacher']) }}
				</template>
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/deduction_items/table','set') || $check_action('/deduction_items/view','set') || $check_action('/deduction_items/view','get') || $check_action('/${table.tableConfig.table_nav_name}/table','add') || $check_action('/${table.tableConfig.table_nav_name}/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/deduction_items/table','set') || $check_action('/deduction_items/view','set') || $check_action('/deduction_items/view','get')"
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/deduction_items/get_list?like=0",
				url_del: "~/api/deduction_items/del?",
			...
			get_list_before(param){
				...
				if(user_group != "管理员"){
						...
						sqlwhere+= "student = " + this.user.user_id + " or ";
																																	...
																																	if(user_group=="教师"){
						sqlwhere+= "rating_teacher = " + this.user.user_id + " or ";
					}
