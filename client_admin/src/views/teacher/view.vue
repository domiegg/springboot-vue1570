			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="状态" prop="state">
					<el-select v-model="form.state" placeholder="请选择">
						...
						<el-col v-if="user_group === '管理员' || $check_field('get','job_no') || $check_field('add','job_no') || $check_field('set','job_no')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="工号" prop="job_no">
												<el-input id="job_no" v-model="form_sub['job_no']" placeholder="请输入工号"
							  v-if="user_group === '管理员' || (form_sub['teacher_id'] && $check_field('set','job_no')) || (!form_sub['teacher_id'] && $check_field('add','job_no'))"></el-input>
					<div v-else-if="$check_field('get','job_no')">{{form_sub['job_no']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="姓名" prop="full_name">
							  ...
							  v-if="user_group === '管理员' || (form_sub['teacher_id'] && $check_field('set','full_name')) || (!form_sub['teacher_id'] && $check_field('add','full_name'))"></el-input>
				...
				<el-form-item label="性别" prop="gender">
						...
						v-if="user_group === '管理员' || (form_sub['teacher_id'] && $check_field('set','gender')) || (!form_sub['teacher_id'] && $check_field('add','gender'))">
				...
				<el-form-item label="校区名称" prop="campus_name">
								<el-select id="campus_name" v-model="form_sub['campus_name']"
						v-if="user_group === '管理员' || (form_sub['teacher_id'] && $check_field('set','campus_name')) || (!form_sub['teacher_id'] && $check_field('add','campus_name'))">
						<el-option v-for="o in list_campus_name" :key="o['campus_name']" :label="o['campus_name']"
							:value="o['campus_name']">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','campus_name')">{{form_sub['campus_name']}}</div>
							</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["teacher_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?teacher_id=" + form_sub["teacher_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/teacher/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/teacher/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/teacher/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/teacher/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/teacher/view','get');
